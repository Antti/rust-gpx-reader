...
pub struct IoReader<T> where T: Read {
    data: T
}

impl <T> IoReader <T> where T: Read {
    ...
    pub fn read_bytes(&mut self, n_bytes: usize) -> Result<Vec<u8>> {
    ...
    pub fn read_short(&mut self) -> Result<i16> {
        self.data.read_i16::<LittleEndian>().map_err(|err| Error::from(err))
    }

    // Read 4 little-endian bytes an integer.
    pub fn read_int(&mut self) -> Result<i32> {
        self.data.read_i32::<LittleEndian>().map_err(|err| Error::from(err))
    }

    // Read 4 little-endian bytes as a float.
    pub fn read_float(&mut self) -> Result<f32> {
        self.data.read_f32::<LittleEndian>().map_err(|err| Error::from(err))
    }

    // Read 8 little-endian bytes as a float.
    pub fn read_double(&mut self) -> Result<f64> {
        self.data.read_f64::<LittleEndian>().map_err(|err| Error::from(err))
    }

    // Read length of the string stored in 1 byte and followed by character bytes.
    pub fn read_byte_sized_string(&mut self, size: usize) -> Result<String> {
    ...
    pub fn read_int_sized_string(&mut self) -> Result<String> {
    ...
    fn read_string(&mut self, size: usize, length: Option<usize>) -> Result<String> {
        ...
        let read_count = try!(self.data.read(&mut buf));
