    ...
    where T: IoReader
{
    ...
    pub fn read(mut self) -> Result<(Version, Song)> {
        ...
        let song = match version {
            ...
            Version::FichierGuitarProV400 |
            Version::FichierGuitarProV406 |
            Version::FichierGuitarProL406 => gp4_reader::read(self.io),
            Version::FichierGuitarProV500 |
            Version::FichierGuitarProV510 => gp5_reader::read(self.io),
        };
        Ok((version, song?))
    }

    fn read_version(&mut self) -> Result<Version> {
        match &self.io.read_byte_sized_string(30)? as &str {
